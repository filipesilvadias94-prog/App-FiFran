...
const [foods,setFoods]=useState([]),[history,setHistory]=useState([]),[settings,setSettings]=useState([]),[water,setWater]=useState([]),[waterRecords,setWaterRecords]=useState([]),[cons,setCons]=useState([]),[records,setRecords]=useState([]),[weights,setWeights]=useState([])
...
async function load(){
 ...
 const [a,b,c,d,wr,e,r,f]=await Promise.all([
  ...
  supabase.from('daily_water_records').select('*').eq('couple_code',CODE).gte('date',ago(30)).order('date'),
 ...
 if(a.error||b.error||c.error||d.error||wr.error||e.error||r.error||f.error)setMsg('Erro ao carregar. Rode o SQL V5 atualizado no Supabase.')
 setFoods(a.data||[]);setHistory(b.data||[]);setSettings(c.data||[]);setWater(d.data||[]);setWaterRecords(wr.data||[]);setCons(e.data||[]);setRecords(r.data||[]);setWeights(f.data||[]);setLoading(false)
}
...
async function registerWater(){
 let total=waterNow()
 if(total<=0)return setMsg('Adicione água antes de registrar.')
 setLoading(true)
 let {error}=await supabase.from('daily_water_records').upsert({couple_code:CODE,profile,date:today(),ml:total},{onConflict:'couple_code,profile,date'})
 if(error){setMsg('Erro ao registrar água. Rode o SQL atualizado.');setLoading(false);return}
 let del=await supabase.from('water_logs').delete().eq('couple_code',CODE).eq('profile',profile).eq('date',today())
 if(del.error)setMsg('Água registrada, mas não consegui zerar o contador.')
 else setMsg(`Água registrada para ${profile}: ${total} ml. O contador foi zerado.`)
 await load()
 setLoading(false)
}
function waterChart(){
 let goal=Number(st(profile).water_goal_ml||2500),arr=[]
 for(let i=6;i>=0;i--){let d=ago(i);let rec=waterRecords.find(x=>x.profile===profile&&x.date===d);arr.push({date:d.slice(5),meta:goal,consumido:Number(rec?.ml||0)})}
 return arr
}
...
<section className="card"><h2>Água de {profile} <span className="badge">{wml}/{wgoal}ml</span></h2><div className="bar"><div style={{width:`${wp}%`}}/></div><div className="grid3"><button onClick={()=>addWater(250)}>+250</button><button onClick={()=>addWater(500)}>+500</button><button onClick={()=>addWater(-250)}>-250</button></div><button onClick={registerWater}>💧 Registrar Água</button><p className="muted">Ao registrar, a água do dia é salva no perfil selecionado e o contador é zerado.</p></section><section className="card"><h2>Meta x Água consumida</h2><div className="chart"><ResponsiveContainer width="100%" height={220}><BarChart data={waterChart()}><CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,.12)"/><XAxis dataKey="date" stroke="#fff"/><YAxis stroke="#fff"/><Tooltip/><Bar dataKey="meta" fill="#45b7ff"/><Bar dataKey="consumido" fill="#3ddc97"/></BarChart></ResponsiveContainer></div></section>
